# About the _Modern C++ Kafka API_

![Lifecycle Active](https://badgen.net/badge/Lifecycle/Active/green)

The [modern-cpp-kafka API](http://opensource.morganstanley.com/modern-cpp-kafka/doxygen/annotated.html) is a layer of **_C++_** wrapper based on [librdkafka](https://github.com/confluentinc/librdkafka) (the **_C_** part only), with high quality, but more friendly to users.
...
The **_librdkafka_** is a robust high performance C/C++ library, widely used and well maintained.

Unfortunately, to maintain **_C++98_** compatibility, the **_C++_** interface of **_librdkafka_** is not quite object-oriented or user-friendly.
...
Eventually, we worked out the **_modern-cpp-kafka_**, -- a **_header-only_** library that uses idiomatic **_C++_** features to provide a safe, efficient and easy to use way of producing and consuming Kafka messages.
...
- **Ease of Use**
  ...
  - **_librdkafka_**'s polling and queue management is now hidden
...
- The compiler should support **_C++17_**

  - Or, **_C++14_**, but with pre-requirements

    - Need **_boost_** headers (for `boost::optional`)

    - For **_GCC_** compiler, it needs optimization options (e.g. `-O2`)
  ...
  - `std::string`
    ...
    - But with exceptions
      ...
      - Ignored options
        ...
        | -------------------------- | ---------------------------------------------------------------------------------- |
        | `enable.auto.offset.store` | **_modern-cpp-kafka_** will save the offsets in its own way                        |
        | `auto.commit.interval.ms`  | **_modern-cpp-kafka_** will only commit the offsets within each `poll()` operation |
...
- Specify library locations with environment variables
  ...
  | -------------------------------- | -------------------------------------------------------- |
  | `LIBRDKAFKA_INCLUDE_DIR`         | **_librdkafka_** headers                                 |
  | `LIBRDKAFKA_LIBRARY_DIR`         | **_librdkafka_** libraries                               |
  | `GTEST_ROOT`                     | **_googletest_** headers and libraries                   |
  | `BOOST_ROOT`                     | **_boost_** headers and libraries                        |
  | `SASL_LIBRARYDIR`/`SASL_LIBRARY` | [optional] for SASL connection support                   |
  | `RAPIDJSON_INCLUDE_DIRS`         | `addons/KafkaMetrics.h` requires **_rapidjson_** headers |
